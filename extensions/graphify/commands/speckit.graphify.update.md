...
2. If `<worktree-path>/graphify-out/` does not exist: log a single-line skip message pointing at `/speckit-graphify-init` and exit 0. **Do not** trigger a full initial build — that is an explicit developer action.
3. If the `graphify` binary is not on `PATH`: log a single-line warning and exit 0.
...
Scope (which files get indexed) is controlled by `<worktree-path>/.graphifyignore` (graphify's native ignore file, gitignore-style). This command always scans the worktree root so `graphify-out/` lands in a stable, predictable location.
