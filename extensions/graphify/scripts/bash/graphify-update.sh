...
#
# Resolves the feature's worktree path, then runs `graphify update <worktree>`
# if and only if (a) graphify-out/ already exists in the worktree and (b) the
# `graphify` binary is on PATH. Non-blocking by design: every failure path exits
# 0 with a warning so the triggering Spec Kit command is not aborted.
#
# Scope is controlled by graphify's native .graphifyignore file at the worktree
# root, NOT by a Spec Kit config knob. graphify writes graphify-out/ next to
# whatever path it's given, so we always pass the worktree root to keep the
# output location stable.
#
...
# 2. Skip if graphify-out/ doesn't exist (init wasn't run).
...
# 4. Run the update non-blockingly. Always scan from worktree root.
