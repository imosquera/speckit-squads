---
description: Portfolio Audit preset for /speckit-analyze. Prepended to the stock command — adds a portfolio-wide audit mode (issues ↔ specs ↔ plans/tasks across main, all worktrees, and specs/archive/) plus a per-feature worktree fallback. The stock per-feature analysis (loaded from the lower-priority template) still runs when this preset's preconditions do not fire.
---
...
2. **Enumerate spec slugs across all worktrees.**
   ...
   - Additionally list `specs/archive/*/` from each worktree — include archived specs in the audit so abandoned-but-resurrected candidates surface.
...
4. **Enumerate GitHub issues — open AND closed.**
   ...
   - Closed issues are intentionally included: closed-but-unspec'd or closed-but-unarchived items reveal cleanup gaps.
...
6. **Detect orphans and dead artifacts.**
   ...
   - Specs that exist under `specs/archive/` but are still referenced by an **open** GitHub issue (archived prematurely).
...
**Table B — Closed issues without any spec or archive entry**

| Issue | Title |

(These are likely missed cleanup — closed without trace.)
...
Kinds include: `tasks-only-scaffold`, `dead-worktree-branch`, `archived-but-open-issue`.
...
- Total open issues
- Total closed issues
- Total spec slugs (incl. archived)
...
- Table B → `/speckit-archive` for confirmed dead work; otherwise reopen the issue.
- Table C → `/speckit-plan` or `/speckit-tasks` inside the worktree that contains the most complete copy.
- Table D → `/speckit-archive` for dead scaffolds; investigate dead worktree branches manually (`git worktree remove`).
